# <img src="./imagens/cat-waving-hand.gif" alt="gif computador com glitch" min-width="45px" max-width="45px" width="45px"> Hello World!

### Full Stack Developer | Laravel • Vue.js • Node.js • React

### Muito prazer, me chamo João Pedro e sou formado em Análise e Desenvolvimento de Sistemas.

<img src="./imagens/notebookGif-crop.gif" alt="gif computador com glitch" min-width="270px" max-width="270px" width="270px" align="right">

Atuo como desenvolvedor Full Stack, trabalhando na construção e manutenção de aplicações, APIs REST, integrações com serviços externos e soluções voltadas para sistemas geográficos.

Tenho experiência principalmente com **Laravel, PHP, Vue.js, Node.js, React, PostgreSQL, Docker e Linux**, além de tecnologias GIS como **OpenLayers, GeoServer e PostGIS**.

Também possuo conhecimentos em outras tecnologias e áreas que fizeram parte da minha formação e de projetos pessoais, como **Angular, Python, C#, Bootstrap, Arduino, AutoCAD e Figma**.

Atualmente busco continuar evoluindo como desenvolvedor Full Stack, trabalhando em projetos que envolvam arquitetura de aplicações, APIs, automações, infraestrutura, GIS e produtos digitais.

---

## 🚀 Tecnologias

### 💻 Front-end

<p>
  <img alt="Tecnologias Front-end" src="https://skillicons.dev/icons?i=vue,react,angular,js,ts,html,css,tailwind,bootstrap" />
</p>

### ⚙️ Back-end

<p>
  <img alt="Tecnologias Back-end" src="https://skillicons.dev/icons?i=php,laravel,nodejs,express,python,cs" />
</p>

### 🗄️ Banco de Dados

<p>
  <img alt="Banco de Dados" src="https://skillicons.dev/icons?i=postgres,mysql,prisma" />
</p>

### 🛠️ DevOps & Ferramentas

<p>
  <img alt="DevOps e Ferramentas" src="https://skillicons.dev/icons?i=docker,linux,git,github,gitlab,figma" />
</p>

### 🧠 Outros conhecimentos

<p>
  <img alt="Outros conhecimentos" src="https://skillicons.dev/icons?i=python,cs,arduino,autocad" />
</p>

* PostGIS
* MySQL
* Modelagem de dados
* Consultas SQL
* CI/CD
* Administração de ambientes de homologação e produção
* Code Review

### 🗺️ GIS & Web Mapping

* OpenLayers
* GeoServer
* PostGIS
* Mapas interativos
* Visualização de dados geográficos
* Integração de dados espaciais com aplicações web

---

## 📫 Meios de Contato

<div align="center" >
<div align="center"> 
  <a href ="https://docs.google.com/document/d/1Tlofrt0tSZGQRdLQZMq2b5WZVhiDW9-ut2HRVoCxUQk/edit?usp=sharing" target="_blank"><img alt="curriculo no drive" src="https://img.shields.io/badge/Curriculo-8A2BE2?style=for-the-badge&logo=googledocs&logoColor=white" target="_blank"></a>
  <a href ="https://api.whatsapp.com/send?phone=5198557211&text=Oi%20Jo%C3%A3o,%20vim%20pelo%20seu%20Github!"><img alt="Meu Whatsapp" src="https://img.shields.io/badge/WhatsApp-25D366?style=for-the-badge&logo=whatsapp&logoColor=white" target="_blank"></a>
  <a href ="mailto:pedjoness28@gmail.com?subject=Ol%C3%A1!"><img alt="Meu Gmail" src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" target="_blank"></a>
  <a href ="https://www.linkedin.com/in/xjoaopedro/"><img alt="Meu Linkedin" src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" target="_blank"></a>
  <a href ="https://www.instagram.com/xjoaopedro__/"><img alt="Meu Instagram" src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" target="_blank"></a>
  <a href="https://portfolio-xjoaopedro.vercel.app">
  <img alt="Portfólio" src="https://img.shields.io/badge/Portfólio-000000?style=for-the-badge&logo=vercel&logoColor=white">
</a>
</div>
</div>

---

<div align="center">

<img src="./imagens/readmeend.gif" alt="gif computador com glitch" min-width="200px" max-width="200px" width="200px">

</div>